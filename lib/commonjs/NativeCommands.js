...
function getViewTag(viewRef) {
  ...
  const nodeHandle = (0, _reactNative.findNodeHandle)(node);
  if (nodeHandle == null) {
 ...
 */
function createCommands(viewRef) {
  return {
    play: () => {
    ...
    loadClip: (clipId, options) => {
      ...
      if (tag != null) {
        ...
        BBPlayerModule?.loadWithClipId(tag, clipId, options?.initiator ?? options?.playout ?? null, options?.autoPlay ?? false, options?.seekTo ?? 0, contextJson);
      }
    },
    // Legacy load methods (kept for backwards compatibility)
    loadWithClipId: (clipId, initiator, autoPlay, seekTo, context) => {
      const tag = getViewTag(viewRef);
      if (tag != null) {
        const contextJson = context ? JSON.stringify(context) : null;
        BBPlayerModule?.loadWithClipId(tag, clipId, initiator ?? null, autoPlay ?? false, seekTo ?? 0, contextJson);
      }
    },
    loadWithClipListId: (clipListId, initiator, autoPlay, seekTo, context) => {
      const tag = getViewTag(viewRef);
      if (tag != null) {
        const contextJson = context ? JSON.stringify(context) : null;
        BBPlayerModule?.loadWithClipListId(tag, clipListId, initiator ?? null, autoPlay ?? false, seekTo ?? 0, contextJson);
      }
    },
    loadWithProjectId: (projectId, initiator, autoPlay, seekTo, context) => {
      const tag = getViewTag(viewRef);
      if (tag != null) {
        const contextJson = context ? JSON.stringify(context) : null;
        BBPlayerModule?.loadWithProjectId(tag, projectId, initiator ?? null, autoPlay ?? false, seekTo ?? 0, contextJson);
      }
    },
    loadWithClipJson: (clipJson, initiator, autoPlay, seekTo, context) => {
      const tag = getViewTag(viewRef);
      if (tag != null) {
        const contextJson = context ? JSON.stringify(context) : null;
        BBPlayerModule?.loadWithClipJson(tag, clipJson, initiator ?? null, autoPlay ?? false, seekTo ?? 0, contextJson);
      }
    },
    loadWithClipListJson: (clipListJson, initiator, autoPlay, seekTo, context) => {
      const tag = getViewTag(viewRef);
      if (tag != null) {
        const contextJson = context ? JSON.stringify(context) : null;
        BBPlayerModule?.loadWithClipListJson(tag, clipListJson, initiator ?? null, autoPlay ?? false, seekTo ?? 0, contextJson);
      }
    },
    loadWithProjectJson: (projectJson, initiator, autoPlay, seekTo, context) => {
      const tag = getViewTag(viewRef);
      if (tag != null) {
        const contextJson = context ? JSON.stringify(context) : null;
        BBPlayerModule?.loadWithProjectJson(tag, projectJson, initiator ?? null, autoPlay ?? false, seekTo ?? 0, contextJson);
      }
    },
    loadWithJsonUrl: (jsonUrl, autoPlay, context) => {
      const tag = getViewTag(viewRef);
      if (tag != null) {
        const contextJson = context ? JSON.stringify(context) : null;
    ...
    getPlayerState: async () => {
      ...
      try {
        ...
        const [state, phase, mode, currentTime, duration, muted, volume, clipData, projectData, playoutData] = await Promise.all([BBPlayerModule.getState?.(tag) ?? null, BBPlayerModule.getPhase?.(tag) ?? null, BBPlayerModule.getMode?.(tag) ?? null, BBPlayerModule.getCurrentTime?.(tag) ?? 0, BBPlayerModule.getDuration?.(tag) ?? 0, BBPlayerModule.getMuted?.(tag) ?? false, BBPlayerModule.getVolume?.(tag) ?? 1, BBPlayerModule.getClipData?.(tag) ?? null, BBPlayerModule.getProjectData?.(tag) ?? null, BBPlayerModule.getPlayoutData?.(tag) ?? null]);
        return {
          state: state ?? "IDLE",
          phase: phase ?? "INIT",
          mode: mode ?? null,
