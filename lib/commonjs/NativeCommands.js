  ...
  return {
    ...
    loadClip: (clipId, options) => {
      ...
      if (tag != null) {
        ...
        BBPlayerModule?.loadWithClipId(tag, clipId, options?.initiator ?? options?.playout ?? null, options?.autoPlay ?? false, options?.seekTo ?? 0);
      }
    },
    ...
    loadWithClipId: (clipId, initiator, autoPlay, seekTo) => {
      const tag = getViewTag(viewRef);
      if (tag != null) {
        BBPlayerModule?.loadWithClipId(tag, clipId, initiator ?? null, autoPlay ?? false, seekTo ?? 0);
      }
    },
    loadWithClipListId: (clipListId, initiator, autoPlay, seekTo) => {
      const tag = getViewTag(viewRef);
      if (tag != null) {
        BBPlayerModule?.loadWithClipListId(tag, clipListId, initiator ?? null, autoPlay ?? false, seekTo ?? 0);
      }
    },
    loadWithProjectId: (projectId, initiator, autoPlay, seekTo) => {
      const tag = getViewTag(viewRef);
      if (tag != null) {
        BBPlayerModule?.loadWithProjectId(tag, projectId, initiator ?? null, autoPlay ?? false, seekTo ?? 0);
      }
    },
    loadWithClipJson: (clipJson, initiator, autoPlay, seekTo) => {
      const tag = getViewTag(viewRef);
      if (tag != null) {
        BBPlayerModule?.loadWithClipJson(tag, clipJson, initiator ?? null, autoPlay ?? false, seekTo ?? 0);
      }
    },
    loadWithClipListJson: (clipListJson, initiator, autoPlay, seekTo) => {
      const tag = getViewTag(viewRef);
      if (tag != null) {
        BBPlayerModule?.loadWithClipListJson(tag, clipListJson, initiator ?? null, autoPlay ?? false, seekTo ?? 0);
      }
    },
    loadWithProjectJson: (projectJson, initiator, autoPlay, seekTo) => {
      const tag = getViewTag(viewRef);
      if (tag != null) {
        BBPlayerModule?.loadWithProjectJson(tag, projectJson, initiator ?? null, autoPlay ?? false, seekTo ?? 0);
      }
    },
    loadWithJsonUrl: (jsonUrl, autoPlay) => {
      const tag = getViewTag(viewRef);
      if (tag != null) {
